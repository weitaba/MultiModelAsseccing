...
# 得到模型对比结果
@retry(3)
def getscore(model, instruction, answers):
    answer_prompt = ""
    for i, answer in enumerate(answers):
        answer_prompt = answer_prompt + f"\n[助手 {chr(65 + i)} 的回答开始]\n{answer}\n[助手 {chr(65 + i)} 的回答结束]"
    print(Fore.GREEN + f"评价模型 {model} 的分析是:\n" + Style.RESET_ALL)
    prompt =  f"""请作为一个公正的评判者，评估人工智能助手对用户指令的回复质量。
评估步骤：
...
在键 “判断” 中表明你的决定，仅当助手 A 占优时使用 “[[A]]”，助手 B 占优时使用 “[[B]]”，助手 B 占优时使用 “[[C]]”，并且不要有任何额外的单词。
注意：
...
""" + answer_prompt
    ...
    match = re.search(r'判断[\s\S]*?([A-Z])', output)
    if match:
        score = match.group(1)
    else:
...
def getbestanswer(models, answers, problem):
    ...
    for j in range(len(models)):
        ...
        score = getscore(models[j], problem, answers) # 得到对比结果(pair为0代表A胜出，为1代表B胜出，平局算B胜出)
        best = ord(score.upper()) - 65
...
def main(models, assess_models, problem, correct_answer):
    ...
    for assess_answer in assess_answers:
        ...
        print(Fore.CYAN + f"根据在{len(models)}个评价模型投票得到的最优回答 \"{best_answer}\"\n被评估模型 {assess_model} 的回答 \"{assess_output}\" 评分为 {score_of_answer}" + Style.RESET_ALL)

    time_end = time.time()  # 结束时间

    spend_time = time_end - time_start  # 计算花费时间

    intput_json = {"problem": problem, "answer": assess_answers, "score": score_of_answers, "correct": correct_answer,
                   "time": spend_time, "standard_model": best_model, "standard_answer": best_answer}
...
if __name__ == '__main__':
    ...
    position_file = 'batch_position.txt'
